  ...
  <div class="detail">
    <div class="detail-view card">
      <div class="data card-body">
        ...
        <img :src="url_image + pokemons.name + '.png'" alt="Erreur : Image non chargée" class="image">
        <h1> 
          {{pokemons.nom}} 
        </h1>
        <h2 v-for="t in pokemons.types" :key="t.type.name" class="type"> 
          {{t.type.name}} 
        </h2>
        <h3 class="abilities"> 
          Talents du Pokémon : 
        ...
        <ul>
          <li v-for="talent in pokemons.abilities" :key="talent.ability.name" class="ability"> 
            {{talent.ability.name}} 
          </li>
        </ul>
        <h3> 
          Taille du Pokémon : {{pokemons.height}} 
        </h3>
        <h3> 
          Poids du Pokémon : {{pokemons.weight}} 
        </h3>
        <h3> 
          Statistiques du Pokémon : 
        </h3>
        <ul>
          <li v-for="statistiques in pokemons.stats" :key="statistiques.base_stat"> 
            {{statistiques.stat.name}} = {{statistiques.base_stat}} 
          </li>
        </ul>
        <ul v-if="pokemons.held_items != 0">
          <li v-for="objet in pokemons.held_items" :key="objet.item.name"> 
            {{objet.item.name}} 
          </li>
        </ul>
      </div>
      <button class="close" v-on:click="fermer_details()">
        Fermer
      </button>
    </div>
  </div>
...
export default {
  ...
  data : function() { return {
    ...
    url_pokemon : "https://pokeapi.co/api/v2/pokemon/" + num + "/",
  };
  },

  created(){
    axios.get(this.url_pokemon).then(response => (this.pokemons = response.data));
  },

  methods: {
    fermer_details() {
      this.$emit("Fermer_image");
      num += 1;
    }
  }
};
